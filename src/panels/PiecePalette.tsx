...
export function PiecePalette() {
  ...
  return (
    <div className="w-[260px] glass-panel flex flex-col overflow-hidden border-r border-black/[0.06]">
      {/* Header */}
      <div className="px-3 pt-3 pb-2">
        ...
        <div className="relative">
          ...
          {search && (
            <button
              onClick={() => setSearch('')}
              className="absolute right-2 top-1/2 -translate-y-1/2 text-[#aeaeb2] hover:text-[#1d1d1f] transition-colors p-0.5"
            >
              <X size={12} />
            </button>
          )}
      ...
      <div className="flex-1 overflow-y-auto pb-2">
        ...
        {filteredCategories.map(([catKey, { label, pieces }]) => {
          ...
          const color = CATEGORY_COLORS[catKey] || '#86868b';
          return (
            <div key={catKey}>
              <button
                onClick={() => !normalizedSearch && toggle(catKey)}
                className="w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-black/[0.02] transition-colors group"
              >
                <span
                ...
                <span className="text-[12px] font-medium flex-1 text-[#1d1d1f]/80 group-hover:text-[#1d1d1f] transition-colors">
                  {label}
                </span>
                <span
                ...
                {!normalizedSearch && (
                  <ChevronDown
                    size={12}
                    className={`text-[#aeaeb2] transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}
                  />
                )}
              </button>
              {isOpen && (
...
function PaletteItem({ definitionId, definition }: { definitionId: string; definition: PieceDefinition }) {
  ...
  return (
    ...
    >
      <GripVertical size={10} className="text-[#d1d1d6] shrink-0" />
      <PieceMiniature definition={definition} />
      <div className="flex-1 min-w-0">
        <div className="text-[11px] font-medium truncate text-[#1d1d1f]">{definition.name}</div>
        <div className="text-[9px] text-[#aeaeb2] flex items-center gap-1.5">
          <span>{definition.widthM}×{definition.heightM}m</span>
          <span className="text-[#d1d1d6]">·</span>
          <span className="text-[#c88800]">{definition.weightKg} kg</span>
        </div>
      </div>
    </div>
  );
  ...
  const renderShape = () => {
    switch (definition.category) {
          ...
          <>
            <defs>
              <linearGradient id={`sg-${definition.heightM}`} x1="0" y1="0" x2="1" y2="0">
                ...
                <stop offset="40%" stopColor="#6e7b8a" />
                <stop offset="100%" stopColor="#4a5a6a" />
              </linearGradient>
            </defs>
            <rect x={cx - tw / 2} y={cy - th / 2} width={tw} height={th} rx={1} fill={`url(#sg-${definition.heightM})`} stroke="#4a5a6a" strokeWidth={0.4} />
            {Array.from({ length: Math.min(rosettes, 5) }, (_, i) => {
              const ry = cy - th / 2 + (i + 0.5) * (th / Math.min(rosettes, 5));
              return <circle key={i} cx={cx} cy={ry} r={1.8} fill="#F2A900" stroke="#c88800" strokeWidth={0.3} />;
            })}
          </>
        );
      }
      case 'ledger': {
        ...
        return (
          <>
            <rect x={cx - bw / 2} y={cy - 1.5} width={bw} height={3} rx={1.5} fill="#6e7b8a" stroke="#4a5a6a" strokeWidth={0.3} />
            <rect x={cx - bw / 2 - 1.5} y={cy - 3} width={3} height={6} rx={0.8} fill="#5a6a7a" stroke="#4a5a6a" strokeWidth={0.3} />
            <rect x={cx + bw / 2 - 1.5} y={cy - 3} width={3} height={6} rx={0.8} fill="#5a6a7a" stroke="#4a5a6a" strokeWidth={0.3} />
          </>
        );
      }
      case 'guardrail': {
        ...
        return (
          <>
            <rect x={cx - bw / 2} y={cy - 1.5} width={bw} height={3} rx={1.5} fill="#F2A900" stroke="#c88800" strokeWidth={0.3} />
            <rect x={cx - bw / 2 - 1} y={cy - 3} width={3} height={6} rx={0.6} fill="#5a6a7a" strokeWidth={0.2} />
            <rect x={cx + bw / 2 - 2} y={cy - 3} width={3} height={6} rx={0.6} fill="#5a6a7a" strokeWidth={0.2} />
          </>
        );
      }
      case 'diagonal': {
        ...
        return (
          <>
            <line x1={cx - dw / 2} y1={cy + dh / 2} x2={cx + dw / 2} y2={cy - dh / 2} stroke="#6e7b8a" strokeWidth={2.5} strokeLinecap="round" />
            <circle cx={cx - dw / 2} cy={cy + dh / 2} r={2} fill="#5a6a7a" stroke="#4a5a6a" strokeWidth={0.3} />
            <circle cx={cx + dw / 2} cy={cy - dh / 2} r={2} fill="#5a6a7a" stroke="#4a5a6a" strokeWidth={0.3} />
          </>
        );
      }
      ...
      case 'baseJack': {
        ...
        return (
          <>
            <rect x={cx - 5} y={cy + rodH / 2 - 2} width={10} height={2} rx={0.5} fill="#5a6a7a" stroke="#4a5a6a" strokeWidth={0.3} />
            <rect x={cx - 1} y={cy - rodH / 2 + 2} width={2} height={rodH - 3} rx={0.5} fill="#6e7b8a" stroke="#5a6a7a" strokeWidth={0.2} />
            <circle cx={cx} cy={cy - rodH / 2 + 2} r={2} fill="#6e7b8a" stroke="#4a5a6a" strokeWidth={0.3} />
          </>
        );
      }
      default: {
